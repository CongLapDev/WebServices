...
import { BaseURL, LOCAL_URL } from "../../api/ApiBase";
        ...
        try {
            ...
            try {
                ...
                console.error("[LOGIN ERROR] loginApi() failed:", {
                    ...
                    fullURL:
                        loginError?.config?.baseURL + loginError?.config?.url,
            ...
            if (!userData) {
                console.error(
                    "[LOGIN ERROR] No user data received from server"
                );
                throw new Error(
                    "Login failed: No user data received from server"
                );
            }

            console.log(
                "[LOGIN] ✓ Login API successful, user ID:",
                userData?.id
            );
            ...
            if (userData && userData.account) {
                if (
                    ...
                    Array.isArray(userData.account.roles)
                ) {
                    const normalizedRoleNames = userData.account.roles
                        ...
                        .filter(Boolean);
                    userData.account.roles = normalizedRoleNames.map(
                        (name) => ({ name })
                    );
                    console.log(
                        "[LOGIN] ✓ Roles normalized:",
                        normalizedRoleNames
                    );
                    console.log(
                        "[LOGIN] Final roles structure:",
                        userData.account.roles
                    );
                } else {
                    console.warn(
            ...
            console.log(
                "[LOGIN] User roles from login response:",
                userRolesRaw
            );
            ...
            if (targetRoute) {
                ...
                if (isNavigatingRef.current) {
                    console.log(
                        "[LOGIN] Navigation already in progress, skipping"
                    );
                    return;
                }
                ...
                const tokenBeforeNav =
                    window.localStorage.getItem("AUTH_TOKEN");
                if (!tokenBeforeNav || tokenBeforeNav.trim() === "") {
                    console.error(
                        "[LOGIN ERROR] Token lost before navigation!"
                    );
                    throw new Error(
                ...
                if (window.location.pathname === targetRoute) {
                    console.log(
                        "[LOGIN] Already on target route, skipping navigation"
                    );
                ...
                setTimeout(() => {
                    ...
                    console.log(
                        "[LOGIN] Current path after navigate():",
                        currentPath
                    );
                    ...
                    if (currentPath === targetRoute) {
                        ...
                        console.error(
                            "[LOGIN] ✗✗✗ NAVIGATION FAILED - Still on /login!"
                        );
                        console.error(
                            "[LOGIN] React Router navigation did not work"
                        );
                        console.error(
                            "[LOGIN] Expected:",
                            targetRoute,
                ...
                setTimeout(() => {
                    ...
                    if (currentPath === targetRoute) {
                        ...
                        console.error(
                            "[LOGIN] React Router navigation completely failed"
                        );
                        console.error(
                        ...
                        console.warn(
                            "[LOGIN] ⚠ User is on different route:",
                            currentPath
                        );
                        console.warn(
                ...
                setTimeout(() => {
                    ...
                    console.log(
                        "[LOGIN] Final path check (800ms delay):",
                        finalPath
                    );
                    if (finalPath !== targetRoute) {
                        console.warn(
                            "[LOGIN] ⚠ Path still not matching after 800ms"
                        );
                        console.warn(
            ...
            if (error?.response) {
                ...
                console.error(
                    "[LOGIN ERROR] - Request URL:",
                    error.config?.url
                );
                console.error(
                    "[LOGIN ERROR] - Request Method:",
                    error.config?.method
                );
                console.error(
                ...
                console.error(
                    "[LOGIN ERROR] - Request Data:",
                    error.config?.data
                );
            } else if (error?.request) {
                // Request was made but no response received
                console.error(
                    "[LOGIN ERROR] Network Error - No response received:"
                );
                console.error("[LOGIN ERROR] - Request:", error.request);
                console.error("[LOGIN ERROR] - This usually means:");
                console.error(
                    "[LOGIN ERROR]   1. Backend server is not running"
                );
                console.error("[LOGIN ERROR]   2. CORS issue");
                console.error(
                    "[LOGIN ERROR]   3. Network connectivity problem"
                );
            } else {
                ...
                console.error("[LOGIN ERROR] - This usually means:");
                console.error(
                    "[LOGIN ERROR]   1. Invalid request configuration"
                );
        ...
        <div className={clsx("w-100", className)}>
            <form noValidate onSubmit={formik.handleSubmit}>
                <Row gutter={[0, 24]} justify={"center"}>
                    ...
                    <Col span={24}>
                        <Row
                            justify={isAdminLogin ? "center" : "end"}
                            gutter={[8, 8]}
                        >
                            <Col
                                ...
                                <Row>
                                    <Col span={6}>
                                        <Button
                                            ...
                                            onClick={() => {
                                                const backendUrl = window.location.hostname === 'localhost' ? LOCAL_URL : BaseURL;
